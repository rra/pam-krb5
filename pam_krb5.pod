...
I<forwardable> to true, I<minimum_uid> to 100, and set I<ignore_k5login>
...
proceed as if they weren't mentioned in the PAM configuration.  Using this
option is highly recommended if you don't need to use Kerberos to
authenticate password logins to the root account (which isn't recommended
since Kerberos requires a network connection).
...
to the local username, provided that either a F<.k5login> file or local
...
built against Heimdal 0.8rc1 or later or the MIT Kerberos PKINIT branch.
...
another, prior module, such as B<pam_cracklib>.
