...
in <format> replaced with the username.  <format> should not contain a
realm; instead, set the I<realm> option to change the default realm.  If
the username contains an C<@>, only the part of the username before the
realm is used to replace C<%s> and the realm of the username is appended
to the result.  There is no quote removal.
