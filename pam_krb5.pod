...
Due to a limitation in PAM's handling of password changes, failure to
change the password in Kerberos due to strength checking in the KDC or
network failures during the password change will not stop the rest of the
PAM password stack from running even if pam_krb5 is marked as requisite
(or the equivalent using the Linux [] syntax).  Be aware of this if you
are using multiple password modules in a stack to synchronize password
